...
Class Mailer {
    ...
    function sendMail() {
        ...
        else
		{
            ...
            $mail->From =  $from;
            $mail->FromName = $from;
            $mail->addAddress($this->toEmail,$this->toEmail);  // Add a recipient
            $mail->addReplyTo($this->replyEmail,$this->replyEmail);
			$mail->WordWrap = (int)$cfg->getConfig("Wordwrap");
			//$mail->addAttachment('/var/tmp/file.tar.gz');         // Add attachments
			$mail->isHTML(true);
			$mail->Subject =$this->contact_subject;// $cfg->getConfig("Subject");
