...
var curHtmlPage = ""
var messages=[{
    'home':'Replacing plastic with bio-based solutions'
    ,'products':'Saving The World For Future Generations'
    ,'about':'About Us'
    ,'news':'What Media Has to Say For Us'
    ,'contact':'Contact Us'
}]
...
function displayData(cur, tagId) {
    ...
    $(".topHeaderCenterContent").html(messages[0][tagId])
...
function getContactEmailQueryURI(id, msgId) {
    ...
    $.get(sPage, function (res) {
        ...
        if (msg.indexOf("err") != -1)
            ...
            $('#contact').removeClass('show overlay').addClass('hide')
        }
        clearForm(id)
    }).error(function (er) {
