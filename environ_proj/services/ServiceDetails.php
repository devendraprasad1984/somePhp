...
//                echo "skipping: $value";
            ...
            if ($withFileContent === "yes") {
                $data = "<div class='box ltqt' style='margin-top: 4px; position: relative'>";
                ...
                if (strpos($value, ".txt")>0)
                    ...
                    $data .= "<div id='obj$this->counter' style='background-color: inherit;'><object data='$fullPath' type='application/pdf' class='pdfView'></object></div>";
                else if (strpos($value, ".png")>0 || strpos($value, ".jpg")>0)
                    $data .= "<div><img src='$fullPath' class='pdfView'/></div>";
                $data .= "</div>";
                ...
                if ($spath === "data") {
                    $data = "<option value='$host?fileRead=$xPath' class='font-weight-bold bg-info ' style='font-size: 13pt;'><div>$label</div></option>";
                } else {
